...
import { AlertTriangle, Plus, ArrowDownRight, ArrowUpRight, ArrowRightLeft } from "lucide-react";
...
export default function Estoque() {
  ...
  return (
    <div className="p-6 md:p-8 h-full overflow-y-auto">
      ...
      {alertas.length > 0 && (
        <div className="mb-8 bg-destructive/10 border border-destructive/20 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center gap-3 text-destructive mb-4">
            <AlertTriangle className="w-6 h-6" />
            <h2 className="text-lg font-bold">Alertas de Estoque Baixo</h2>
          </div>
              ...
              <table className="w-full text-left">
                <thead className="bg-muted/50 text-muted-foreground text-sm uppercase">
                  <tr>
                    ...
                    <th className="px-6 py-4 font-medium">Estoque Min.</th>
                    <th className="px-6 py-4 font-medium text-right">Estoque Atual</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {produtos.map(p => (
                    <tr key={p.id} className="hover:bg-muted/30 transition-colors">
                      <td className="px-6 py-4 font-medium">{p.nome} {p.marca ? `- ${p.marca}` : ''}</td>
                      <td className="px-6 py-4 capitalize">{p.categoria}</td>
                      <td className="px-6 py-4 text-muted-foreground">{p.estoque_min} {p.unidade}</td>
                      <td className={`px-6 py-4 text-right font-bold font-mono ${p.estoque <= p.estoque_min ? 'text-destructive' : 'text-primary'}`}>
                        {p.estoque} {p.unidade}
                      </td>
                    </tr>
                  ))}
              ...
              {movimentos.map(m => (
                <div key={m.id} className="flex gap-4 p-4 rounded-xl border border-border bg-secondary/10">
                  <div className={`p-3 rounded-xl flex items-center justify-center ${
                    ...
                    m.tipo === 'saida' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'
                  }`}>
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      ...
                      <span className={`font-mono font-bold ${
                        ...
                        m.tipo === 'saida' ? 'text-red-600' : 'text-blue-600'
                      }`}>
